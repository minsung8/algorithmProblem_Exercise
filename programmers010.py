...
def solution(word, pages):
    ...
    for page in pages:
        url_idx = page.index('<meta property="og:url" content=')
        temp_url = page[url_idx:].split('"')[3]
    ...
    for key in url_dic.keys():
        ...
        for i in range(len(temp_str)):
            if flag and temp_str[i].isalpha():
                temp_word += temp_str[i]
            elif flag and not temp_str[i].isalpha() and len(temp_word) > 0:
            ...
            if i > 0 and temp_str[i - 1] == 'a' and temp_str[i] == '>':
                flag = True
        score_dic[key] += word_list.count(word)
    answer_list = []
    for key in url_dic:
